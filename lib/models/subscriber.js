...
module.exports.delete = function deleteSubscriber(to, callback) {
  ...
  datastore.runQuery(query, (err, entities, nextQuery) => {
    if (err) {
      logme.error('datastore error during find for delete: ' + err);
      return callback(err);
    }
    ...
    datastore.delete(keys, function (err) {
      if (err) {
        logme.error('datastore error during delete subscribers: ' + err);
        return callback(err);
      }

      logme.info('successfully deleted subscriber(s)');   
      callback();
    });
  });
}
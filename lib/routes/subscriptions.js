...
router.post('/subscribe', function (req, res, next) {
	...
	Joi.validate(req.body, requestSchema, (err, value) => {
		...
		async.series([
					...
					async.series([
						...
						function sendForecast(nextSmsMsg) {
							...
							forecastClient.getForecastByZipCode(value.zip, function (err, forecast) {
								if (err) {
									winston.error('Error fetching forecast for zip ' + value.zip + ': ' + err);
									return nextSmsMsg();
								}

								winston.info('Successful forecast for ' + value.to + ' for zip ' + value.zip);
			...
			if (err) {
				winston.error('Error during subscribe: ' + err);
...
router.get('/unsubscribe', function (req, res, next) {
	...
	Joi.validate(req.query, requestSchema, (err, value) => {
		...
		async.series([
			...
			if (err) {
				winston.error('Error during unsubscribe: ' + err);
...
router.post('/sms/message', function (req, res, next) {
	...
	if( process.env.TWILIO_SID != req.body.AccountSid ) {
		winston.error('Invalid sender. It did not come from Twilio');
		res.sendStatus(500);
		return;
	}
	const MessagingResponse = twilio.twiml.MessagingResponse;
	...
	if( msg === 'stop' ) {
		winston.info('turning off ' + phone);
		subscriber.delete(phone_subscriber, function(err) {
			if (err) {
				winston.error("Error during sms subscriber delete: " + err);
			...
			sms.renderUnsubscribe(phone_subscriber, function(err, text) {
				if (err) {
					winston.error("Error rendering sms unsubscribe template: " + err);
		...
		if( ZIP_REGEX.test(zip) ) {
				...
				if (err) {
					...
					async.parallel({
						...
						forecast: function (done) {
								...
								if (err) {
									winston.error('Error fetching forecast for zip ' + zip + ': ' + err);
									return done();
								}

								if (!forecast.properties) {
									winston.error('missing forecast.properties: ' + forecast);
									winston.error(forecast);
									return done();
								}
								winston.info('Successful forecast for ' + phone_subscriber + ' for zip ' + zip);
								if (forecast.properties.code === 'low') {
									winston.debug('skipping good forecast: ');
									return done();
								}
						...
						if (err) {
							winston.error('Error rendering sms welcome messages: ' + err);
							return res.send(response.toString());
						}
						...
						winston.info(response.toString());
						return res.send(response.toString());
					});
				}
			});
			...
			winston.info("Invalid zipcode request: " + response.toString());
			res.send(response.toString());
		}
	} else {
		message.body("Hmm. I don't understand. You can send START <zipcode> or STOP to turn it off.");
		winston.info("Invalid SMS command: " + response.toString());
		res.send(response.toString());
	}
});

...
function send(toEmail, name, templateResult, callback) {
    var data = {
        from: 'Propeller <help@propellerhealth.com>',
        to: toEmail,
        subject: templateResult.subject,
        text: templateResult.text,
        html: templateResult.html,
        'o:tag': "asthma-forecast:"+name
      };
...
function render(name, toEmail, templateVars, callback) {
    ...
    template.render(templateVars, function(err, result){
        if(err) {
            ...
            send(toEmail, name, result, function(err) {
                callback(err);
            });
        }
    });
